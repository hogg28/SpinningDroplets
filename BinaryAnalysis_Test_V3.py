# -*- coding: utf-8 -*-
"""
Created on Mon Feb 13 13:05:06 2023
...
def find_edges(frame, idx):
    ...
    for x in range(0,len(frame[0])):
        ...
        edge_results = edge_results.append([{'peak': peak.max(),
                                              'x_val': x,
                                              'frame': idx,
                                              },])
    return edge_results
...
list_of_directories = pd.read_csv('C:/Users/johna/OneDrive/Documents/GradSchool/KDVLab/SpinningDrops/ListofDirectories_10.csv').directory
prefix = '/*.tiff'
...
def find_surface_area(frame,idx):
    ...
    i = -1
    while r > 5500 or r < 3000 or yc > 0:
        i+=1
        print(i)
    ...
    for i in range(len(binary[0])):
        for j in edges[edges.x_val==i].y_lens:
            binary[int(j):int(edges[edges.x_val==i].peak), i] = False
    ...
    size = sizes[sizes>10000]
    objs = ndimage.find_objects(mask)

    # Get the height and width
    height = int(objs[0][0].stop - objs[0][0].start)
    width = int(objs[0][1].stop - objs[0][1].start)
    y1 = int(objs[0][0].start)
    y2 = int(objs[0][0].stop)
    x1 = int(objs[0][1].start)
    x2 = int(objs[0][1].stop)
    mask = np.invert(mask)
    
    plt.figure()
    plt.imshow(mask)
    plt.plot((x1,x1),(y1,y2), c = 'C0')
    plt.plot((x2,x2),(y1,y2), c = 'C0')
    plt.plot((x1,x2),(y1,y1), c = 'C0')
    plt.plot((x1,x2),(y2,y2), c = 'C0')
    # plt.plot((objs[0][0].stop,objs[0][0].stop),(objs[0][1].start, objs[0][1].stop), c = 'C1')
    # plt.plot((objs[0][0].start,objs[0][0].stop),(objs[0][1].start, objs[0][1].start), c = 'C1')
    # plt.plot((objs[0][0].start,objs[0][0].stop),(objs[0][1].stop, objs[0][1].stop), c = 'C1')
    ...
    #                                                    'width': width,
    #                                                    },])
    Aggregate_structure = Aggregate_structure.append([{'size': float(size.max()),
...
for list_of_dir in list_of_directories:
    ...
    data_directory = '/'.join(text)+'/Data_9.csv'
